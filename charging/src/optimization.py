...
class PHEVCSP():
	...
	def ProcessItinerary(self,
		...
		work_charger_likelihood=1,
		destination_charger_likelihood=.1,
		consumption_cd=478.8,
		consumption_cs=478.8*5,
		battery_capacity=40*3.6e6,
		fuel_tank_capacity=400*3.6e6,
		initial_soc=.5,
		final_soc=.5,
		initial_sof=.5,
		final_sof=.5,
		ad_hoc_charger_power=14.15e6,
		home_charger_power=12.1e3,
		work_charger_power=12.1e3,
		destination_charger_power=12.1e3,
		max_soc=1,
		...
		min_sof=.1,
		min_dwell_event_duration=15*60,
		max_ad_hoc_event_duration=10*60,
		min_ad_hoc_event_duration=.1*60,
		payment_penalty=60,
		travel_penalty=15*60,
		dwell_charge_time_penalty=0,
		ad_hoc_charge_time_penalty=1,
		...
		**kwargs,
		...
		trip_times=np.tile(durations,tiles)*60 #[s]
		trip_mean_speeds=trip_distances/trip_times #[m/s]
		trip_discharge_cd=trip_distances*consumption_cd #[J]
		trip_discharge_cs=trip_distances*consumption_cs #[J]
		...
		inputs['n_e']=n_e
		inputs['s_c_i']=initial_soc*battery_capacity
		inputs['s_c_f']=final_soc*battery_capacity
		inputs['s_c_ub']=max_soc*battery_capacity
		inputs['s_c_lb']=min_soc*battery_capacity
		inputs['s_f_i']=initial_sof*fuel_tank_capacity
		inputs['s_f_f']=final_sof*fuel_tank_capacity
		inputs['s_f_ub']=max_sof*fuel_tank_capacity
		inputs['s_f_lb']=min_sof*fuel_tank_capacity
		inputs['c_db']=np.ones(n_e)*payment_penalty*is_other
		inputs['c_dd']=np.ones(n_e)*dwell_charge_time_penalty
		inputs['c_ab']=np.ones(n_e)*(travel_penalty+payment_penalty)
		inputs['c_ad']=np.ones(n_e)*ad_hoc_charge_time_penalty
		inputs['r_d']=destination_charger_power_array
		# inputs['r_d']=np.ones(n_e)*destination_charger_power
		inputs['r_a']=np.ones(n_e)*ad_hoc_charger_power
		inputs['d_e_min']=np.ones(n_e)*min_dwell_event_duration
		inputs['d_e_max']=dwells
		inputs['a_e_min']=np.ones(n_e)*min_ad_hoc_event_duration
		inputs['a_e_max']=np.ones(n_e)*max_ad_hoc_event_duration
		inputs['d_cd']=trip_discharge_cd
		inputs['d_cs']=trip_discharge_cs
		inputs['b_c']=battery_capacity
		inputs['f_c']=fuel_tank_capacity
		inputs['l_i']=trip_distances.sum()
		inputs['is_home']=is_home
		inputs['is_work']=is_work
		inputs['is_other']=is_other
	...
	def Variables(self):
		...
		self.model.u_cd_cs=pyomo.Var(self.model.E,domain=pyomo.PercentFraction)
	...
	def Bounds(self):
		...
		charge_level=self.inputs['s_c_i']
		fuel_level=self.inputs['s_f_i']

		for e in range(self.inputs['n_e']):

			d_cd=self.inputs['d_cd'][e]*(self.model.u_cd_cs[e])
			d_cs=self.inputs['d_cs'][e]*(1-self.model.u_cd_cs[e])
			...
			self.model.bounds.add((self.inputs['s_c_lb'],charge_level,
				self.inputs['s_c_ub']))

			self.model.bounds.add((self.inputs['s_f_lb'],fuel_level,
				self.inputs['s_f_ub']))

		self.model.fs.add(expr=charge_level>=self.inputs['s_c_f'])
		self.model.fs.add(expr=fuel_level>=self.inputs['s_f_f'])
	...
	def Compute(self):
		...
		# print(self.solution['u_cd_cs'])
		# print(self.solution['u_dd'][0]*self.inputs['r_d'])
		# print(self.solution['u_dd'][0],self.inputs['c_db'])

		charge_level=np.append(
			self.inputs['s_c_i'],
			self.inputs['s_c_i']+
			np.cumsum(-self.inputs['d_cd']*(self.solution['u_cd_cs'][0]))+
			np.cumsum(self.solution['u_dd'][0]*self.inputs['r_d'])
			)

		soc=charge_level/self.inputs['b_c']

		fuel_level=np.append(
			self.inputs['s_f_i'],
			self.inputs['s_f_i']+
			np.cumsum(-self.inputs['d_cs']*(1-self.solution['u_cd_cs'][0]))+
			np.cumsum(self.solution['u_ad'][0]*self.inputs['r_a'])
			)

		sof=fuel_level/self.inputs['f_c']
		...
		self.solution['soc',0]=soc[1:]
		self.solution['sof',0]=sof[1:]

		# self.solution['sic',0]=self.sic
...
class ICEVCSP():
	...
	def ProcessItinerary(self,
		...
		max_sof=1,
		min_sof=.1,
		max_ad_hoc_event_duration=15*60,
		min_ad_hoc_event_duration=.1*60,
		payment_penalty=60,
		travel_penalty=5*60,
		tiles=7,
